...
export default function PainelPrecoProdutos() {
  ...
  useEffect(() => {
    async function fetchProdutos() {
      const roupasSnap = await getDocs(collection(db, "roupas"))
      const acessoriosSnap = await getDocs(collection(db, "acessorios"))
      let roupasArr = roupasSnap.docs.map(doc => ({ id: Number(doc.id), ...doc.data() }))
      let acessoriosArr = acessoriosSnap.docs.map(doc => ({ id: Number(doc.id), ...doc.data() }))
      // Se Firestore estiver vazio, popular com produtos padrão
      if (roupasArr.length === 0) {
        roupasArr = produtosPadrao
        for (const prod of produtosPadrao) {
          await setDoc(doc(db, "roupas", String(prod.id)), prod)
        }
      }
      if (acessoriosArr.length === 0) {
        acessoriosArr = acessorios
        for (const prod of acessorios) {
          await setDoc(doc(db, "acessorios", String(prod.id)), prod)
        }
      }
      setRoupas(roupasArr)
      setAcessoriosList(acessoriosArr)
    }
    fetchProdutos()
  }, [])
  ...
  async function handleSave(tipo: "roupa" | "acessorio", idx: number, id: number) {
    if (tipo === "roupa") {
      ...
      await setDoc(doc(db, "roupas", String(id)), novos[idx])
    } else {
      ...
      await setDoc(doc(db, "acessorios", String(id)), novos[idx])
    }
  }
